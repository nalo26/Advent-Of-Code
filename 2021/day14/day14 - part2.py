...
for _ in range(40):
    ...
    for pair in occurences:
        rule = rules[pair]
        if pair[0] + rule not in newOccurences:
            newOccurences[pair[0] + rule] = 0
        newOccurences[pair[0] + rule] += occurences[pair]
        if rule + pair[1] not in newOccurences:
            newOccurences[rule + pair[1]] = 0
...
secnd_letter = {}
for pair in occurences:
    if pair[0] not in first_letter:
        first_letter[pair[0]] = 0
    first_letter[pair[0]] += occurences[pair]
    if pair[1] not in secnd_letter:
        secnd_letter[pair[1]] = 0
    secnd_letter[pair[1]] += occurences[pair]

_count = {
    x: max(first_letter.get(x, 0), secnd_letter.get(x, 0))
    for x in set(first_letter) | set(secnd_letter)
}
